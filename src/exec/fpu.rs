...
pub enum Source<'a> {
    /// Sources the value from a built-in `f64` value.
    Builtin(f64),
    ...
    /// This is needed to efficiently copy values, because `Extended` would borrow the `FPU`.
...
impl FPU {
    ...
    pub fn get_tag(val: &Float) -> TagValue {
        if val.is_infinite() || val.is_nan() { TagValue::Special }
        else if val.is_zero() { TagValue::Zero }
        else { TagValue::NonZero }
    }
    ...
    pub fn assign_physical(&mut self, index: u8, src: Source) -> Result<(), ()> {
        match src {
            Source::Builtin(v) => self.vals[index as usize].assign(v),
            Source::Extended(v) => self.vals[index as usize].assign(v),
            Source::Physical(v) => {
...
fn test_fpu() {
    ...
    let i = fpu.push(Source::Builtin(3.78)).unwrap();
    ...
    let j = fpu.push(Source::Extended(&Float::with_val(MAX_PRECISION * 2, &*E))).unwrap();
    ...
    let k = fpu.push(Source::Extended(&Float::with_val(MAX_PRECISION / 2, Constant::Pi))).unwrap();
    ...
    {
        let x = fpu.push(Source::Builtin(f64::INFINITY)).unwrap();
    ...
    {
        let x = fpu.push(Source::Builtin(f64::NEG_INFINITY)).unwrap();
        assert_eq!(fpu.tag.get_physical(x), TagValue::Special as u8);
        let val = &fpu.vals[x as usize];
    ...
    {
        let x = fpu.push(Source::Builtin(f64::NAN)).unwrap();
        assert_eq!(fpu.tag.get_physical(x), TagValue::Special as u8);
        let val = &fpu.vals[x as usize];
        assert!(val.is_nan());
        assert_eq!(fpu.pop().unwrap(), x);
    ...
    fpu.assign_physical(i, Source::Physical(i)).unwrap_err();
    fpu.assign_physical(k, Source::Physical(i)).unwrap_err();
    assert!((fpu.vals[i as usize].to_f64() - 3.78).abs() <= 0.00000000001);
    assert!((fpu.vals[j as usize].to_f64() - 2.718281828459045235360).abs() <= 0.00000000001);
    assert!((fpu.vals[k as usize].to_f64() - 3.1415926).abs() <= 0.000001);
    ...
    fpu.assign_physical(above, Source::Builtin(-2.6)).unwrap();
    assert_eq!(fpu.tag.get_physical(above), TagValue::NonZero as u8);
    assert!((fpu.vals[above as usize].to_f64() - -2.6).abs() <= 0.000001);

    fpu.push(Source::Builtin(5.6)).unwrap_err();
    assert_eq!(fpu.status.get_top(), k);
    assert_eq!(fpu.tag.get_physical(above), TagValue::NonZero as u8);
    assert!((fpu.vals[k as usize].to_f64() - 3.1415926).abs() <= 0.000001);
    assert!((fpu.vals[above as usize].to_f64() - -2.6).abs() <= 0.000001);

    fpu.assign_physical(k, Source::Physical(above)).unwrap();
    assert_eq!(fpu.status.get_top(), k);
    assert_eq!(fpu.tag.get_physical(above), TagValue::NonZero as u8);
    assert_eq!(fpu.tag.get_physical(k), TagValue::NonZero as u8);

...
#[test]
fn temp_bin_escapes_asm_time() {
    // remove this (entire) test when $bin is updated to allow (some) link-time uses
    // replace it with the actually bad cases...

    match assemble("test.asm", &mut "extern msg\nval: equ $bin(msg)".as_bytes(), Default::default()) {
        Ok(_) => panic!(),
        Err(e) => {
            match e.kind {
                AsmErrorKind::BinaryInternEscapesAsmTime => (),
                _ => panic!("{:?}", e),
            }
            assert_eq!(e.line_num, 2);
            assert_eq!(e.pos, None);
    ...
    match assemble("test.asm", &mut "extern msg\nsegment text\nmov rdi, $bin(msg)".as_bytes(), Default::default()) {
        Ok(_) => panic!(),
        Err(e) => {
            match e.kind {
                AsmErrorKind::BinaryInternEscapesAsmTime => (),
    ...
    match assemble("test.asm", &mut "extern msg\nsegment rodata\nval: dq $bin(msg)".as_bytes(), Default::default()) {
        Ok(_) => panic!(),
        Err(e) => {
            match e.kind {
                AsmErrorKind::BinaryInternEscapesAsmTime => (),
